...
class JsonParserUtils {
        ...
        try {
            ...
            for (i in 0 until length) {
                ...
                category.defaultName =
                    parseJsonObject(jsonObjectCategory, KEY_CATEGORY_DEFAULT_NAME)
    ...
    fun bindJsonToNewsItemModel(jsonArray: JSONArray): ArrayList<NewsItem> {
        ...
        try {
            ...
            for (i in 0 until length) {
                ...
                val jsonObjectImage =
                    parseJsonObjectFromObject(jsonObjectNewsItem, KEY_NEWS_ITEM_IMAGE)
                if (jsonObjectImage != null) {
                    newsItem.photo = parseJsonObject(jsonObjectImage, KEY_NEWS_ITEM_PHOTO)
                    newsItem.photoCaption =
                        parseJsonObject(jsonObjectImage, KEY_NEWS_ITEM_PHOTO_CAPTION)
    ...
    fun bindJsonToSubSection(jsonArray: JSONArray): ArrayList<Subsection> {
        val subSectionList = ArrayList<Subsection>()
        try {
            val length = jsonArray.length()
            for (i in 0 until length) {
                val jsonObjectSubsection = parseJsonObjectFromArray(jsonArray, i)
                val subsection = Subsection()
                subsection.category =
                    parseJsonObject(jsonObjectSubsection!!, KEY_SUBSECTION_CATEGORY)
                subsection.url = parseJsonObject(jsonObjectSubsection, KEY_SUBSECTION_URL)
    ...
    companion object {
        ...
        //subsection
        const val KEY_JSON_ARRAY_SUBSECTION_ITEM = "Item"
        const val KEY_SUBSECTION_CATEGORY = "category"
        const val KEY_SUBSECTION_URL = "url"
